...
INT_PTR CALLBACK TextStuff(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        if (LOWORD(wParam) == IDC_BUTTON1) {
            ...
            tHandler->increaseChance(0);
        }
        if (LOWORD(wParam) == IDC_BUTTON2) {
            //pn and ps
            MessageBeep(MB_ICONASTERISK);
            tHandler->increaseChance(1);
        }
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONDOWN: {
        DialogBox(hInst, MAKEINTRESOURCE(IDD_DIALOG1), hWnd, TextStuff);
        break;
    }
    case WM_CREATE:
